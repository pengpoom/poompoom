...
export function ServicePathsSection({
  ...
  return (
    <ConfigSection title="服务与路径" description="发布版默认从可执行文件同级的 data/ 和 static/ 读写配置与静态资源，路径通常不建议频繁修改。">
            ...
            items={[
              {
                ...
                body: (
                  <>
                    <code>static</code>，或者绝对路径如 <code>D:\\Image-Studio\\static</code>。
                  </>
                ),
              },
              {
      ...
      <ToggleField
        label="同步复用代理"
        hint="开启后远端同步请求也会复用同一代理。"
        tooltip={
          <TooltipDetails
            items={[
              {
                title: "开启后",
                body: <>远端同步相关请求也会复用同一条代理，适合管理接口也需要代理出网的情况。</>,
              },
              {
                title: "关闭后",
                body: <>同步管理请求会直接走本机网络。</>,
              ...
              {
                title: "注意",
                body: <>这个开关只影响同步管理请求，不影响业务生图的 API 接入。</>,
              },
            ]}
          />
        }
        ...
        tooltip={
          <TooltipDetails
            items={[
              ...
              {
                title: "作用",
                body: <>上游管理页导入的 auth 文件会主要落在这里，后端读取号池时也会从这里扫描。</>,
              },
              {
                title: "改动风险",
        ...
        tooltip={
          <TooltipDetails
            items={[
              ...
              {
                title: "作用",
                body: <>本地与远端的同步记录、差异状态和中间状态会写到这里。</>,
              },
              {
                title: "删除或改路径后",
