...
func TestQueuedImageTaskExpiresBeforeFirstRun(t *testing.T) {
	...
	server.imageTasks.mu.Lock()
	queuedTask := server.imageTasks.tasks["turn-expire-queued"]
	if queuedTask == nil {
		server.imageTasks.mu.Unlock()
		t.Fatal("queued task missing")
	}
	queuedTask.CreatedAt = time.Now().Add(-internalCompatImageTaskQueueTTL - time.Second)
	server.imageTasks.mu.Unlock()
	server.imageTasks.triggerSchedule()
