...
func TestNormalizeRootPrefersExecutableConfigRoot(t *testing.T) {
	...
	osExecutable = func() (string, error) {
		return filepath.Join(releaseDir, "image-studio.exe"), nil
	}
...
func TestValidateMigratesLegacyCPAImageRoutesToStudioRoutes(t *testing.T) {
	cfg := &Config{
		ChatGPT: ChatGPTConfig{
			ImageMode:      "studio",
			FreeImageRoute: "images_api",
			FreeImageModel: "gpt-image-2",
			PaidImageRoute: "codex_responses",
	...
	if cfg.ChatGPT.PaidImageRoute != "legacy" {
		t.Fatalf("PaidImageRoute = %q, want legacy", cfg.ChatGPT.PaidImageRoute)
	}
}
...
func TestValidateDefaultsEmptyStoragePaths(t *testing.T) {
	cfg := &Config{
		ChatGPT: ChatGPTConfig{
			ImageMode:      "studio",
			FreeImageRoute: "legacy",
	...
	if cfg.Storage.AuthDir != "data/auths" {
		t.Fatalf("AuthDir = %q, want data/auths", cfg.Storage.AuthDir)
	}
	if cfg.Storage.StateFile != "data/accounts_state.json" {
		t.Fatalf("StateFile = %q, want data/accounts_state.json", cfg.Storage.StateFile)
	}
	if cfg.Storage.SyncStateDir != "data/sync_state" {
		t.Fatalf("SyncStateDir = %q, want data/sync_state", cfg.Storage.SyncStateDir)
	}
	...
	if cfg.Storage.RedisPrefix != "imagestudio:studio" {
		t.Fatalf("RedisPrefix = %q, want imagestudio:studio", cfg.Storage.RedisPrefix)
	}
}
