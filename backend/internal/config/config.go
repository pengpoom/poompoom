...
import (
	...
	"imagestudio/internal/outboundproxy"
...
func (c *Config) validate() error {
	...
	if strings.TrimSpace(c.Storage.StateFile) == "" {
		c.Storage.StateFile = "data/accounts_state.json"
	}
	if strings.TrimSpace(c.Storage.SyncStateDir) == "" {
		c.Storage.SyncStateDir = "data/sync_state"
	}
	...
	if strings.TrimSpace(c.Storage.RedisPrefix) == "" {
		c.Storage.RedisPrefix = "imagestudio:studio"
	}
...
func normalizeImageRoute(route string) (string, bool) {
	...
	case "images_api", "codex_responses", "auto":
		return "legacy", true
	default:
		return "", false
	}
...
func normalizeAPIAccessPlatform(value string) string {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "", "gpt-image":
		return "gpt-image"
	case "gemini-banana":
