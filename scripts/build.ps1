...
$ldflags = @(
  "-s",
  "-w",
  "-X imagestudio/internal/buildinfo.Version=$($buildMeta.Version)",
  "-X imagestudio/internal/buildinfo.Commit=$($buildMeta.Commit)",
  "-X imagestudio/internal/buildinfo.BuildTime=$($buildMeta.BuildTime)"
) -join " "
...
go build -ldflags $ldflags -o (Join-Path $distDir "image-studio.exe") .
...
Copy-Item -Path (Join-Path $distDir "image-studio.exe") -Destination (Join-Path $packageDir "image-studio.exe") -Force
